...
DEPENDENCIES = ['i2c', 'text_sensor', 'binary_sensor']
...
CONFIG_SCHEMA = cv.Schema({
    ...
    cv.Optional(CONF_FAULT_SENSOR): cv.use_id(binary_sensor.BinarySensor),
...
async def to_code(config):
    ...
    if CONF_FAULT_SENSOR in config:
        sens = await cg.get_variable(config[CONF_FAULT_SENSOR])
